...
# AFFECTIVE_ENGINE ABI/FFI Documentation
...
│  generated/abi/affective_engine.h                │
└─────────────────┬───────────────────────────┘
                  │
                  │ imported by
                  ▼
...
└─────────────────┬───────────────────────────┘
                  │
                  │ compiled to libaffective_engine.so/.a
                  ▼
┌─────────────────────────────────────────────┐
...
│       └── include/
│           └── affective_engine.h   # C header (optional, can be generated)
│
├── generated/                  # Auto-generated files
│   └── abi/
│       └── affective_engine.h       # Generated from Idris2 ABI
│
...
```bash
cd src/abi
idris2 --cg c-header Types.idr -o ../../generated/abi/affective_engine.h
```
...
```c
#include "affective_engine.h"

int main() {
    void* handle = affective_engine_init();
    if (!handle) return 1;

    int result = affective_engine_process(handle, 42);
    if (result != 0) {
        const char* err = affective_engine_last_error();
        fprintf(stderr, "Error: %s\n", err);
    }

    affective_engine_free(handle);
    return 0;
}
```
...
Compile with:
```bash
gcc -o example example.c -laffective_engine -L./zig-out/lib
```

### From Idris2

```idris
import AFFECTIVE_ENGINE.ABI.Foreign
...
```rust
#[link(name = "affective_engine")]
extern "C" {
    fn affective_engine_init() -> *mut std::ffi::c_void;
    fn affective_engine_free(handle: *mut std::ffi::c_void);
    fn affective_engine_process(handle: *mut std::ffi::c_void, input: u32) -> i32;
}

fn main() {
    unsafe {
        let handle = affective_engine_init();
        assert!(!handle.is_null());

        let result = affective_engine_process(handle, 42);
        assert_eq!(result, 0);
...
```julia
const libaffective_engine = "libaffective_engine"

function init()
    handle = ccall((:affective_engine_init, libaffective_engine), Ptr{Cvoid}, ())
    handle == C_NULL && error("Failed to initialize")
    handle
end

function process(handle, input)
    result = ccall((:affective_engine_process, libaffective_engine), Cint, (Ptr{Cvoid}, UInt32), handle, input)
    result
end

function cleanup(handle)
    ccall((:affective_engine_free, libaffective_engine), Cvoid, (Ptr{Cvoid},), handle)
end
...
2. **Generate C header**
   ```bash
   idris2 --cg c-header src/interface/Abi/Types.idr -o generated/abi/affective_engine.h
   ```
