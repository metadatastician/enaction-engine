...
# ENACTION_ENGINE ABI/FFI Documentation
...
│  generated/abi/enaction_engine.h                │
└─────────────────┬───────────────────────────┘
                  │
                  │ imported by
                  ▼
...
└─────────────────┬───────────────────────────┘
                  │
                  │ compiled to libenaction_engine.so/.a
                  ▼
┌─────────────────────────────────────────────┐
...
│       └── include/
│           └── enaction_engine.h   # C header (optional, can be generated)
│
├── generated/                  # Auto-generated files
│   └── abi/
│       └── enaction_engine.h       # Generated from Idris2 ABI
│
...
### 3. **Bounded Cross-Compilation**

The accelerator ABI v1 layout is defined for 64-bit-pointer targets. The Zig
implementation rejects narrower targets at compile time. These examples are
intended target forms, not evidence that every OS/architecture combination has
been built or that compiler layout refines the Idris model:
...
`wasm32` is not an accelerator ABI v1 target. It remains relevant to the
separate SNIF and determinism work, whose wire boundary must not be conflated
with this native pointer-bearing ABI.
...
Idris2 0.7 does not provide a `c-header` code generator. `Abi.Generate` is the
typechecked deterministic renderer; the check command regenerates C-compatible,
Zig and Rust declarations and compares them byte-for-byte.

### Cross-Compile

Only 64-bit-pointer targets are admitted by accelerator ABI v1. A successful
cross-build is build evidence for that target, not a formal platform proof.
...
```c
#include "enaction_engine.h"

int main() {
    void* handle = enaction_engine_init();
    if (!handle) return 1;

    int result = enaction_engine_process(handle, 42);
    if (result != 0) {
        const char* err = enaction_engine_last_error();
        fprintf(stderr, "Error: %s\n", err);
    }

    enaction_engine_free(handle);
    return 0;
}
```
...
Compile with:
```bash
gcc -o example example.c -lenaction_engine -L./zig-out/lib
```

### From Idris2

```idris
import ENACTION_ENGINE.ABI.Foreign
...
```rust
#[link(name = "enaction_engine")]
extern "C" {
    fn enaction_engine_init() -> *mut std::ffi::c_void;
    fn enaction_engine_free(handle: *mut std::ffi::c_void);
    fn enaction_engine_process(handle: *mut std::ffi::c_void, input: u32) -> i32;
}

fn main() {
    unsafe {
        let handle = enaction_engine_init();
        assert!(!handle.is_null());

        let result = enaction_engine_process(handle, 42);
        assert_eq!(result, 0);
...
```julia
const libenaction_engine = "libenaction_engine"

function init()
    handle = ccall((:enaction_engine_init, libenaction_engine), Ptr{Cvoid}, ())
    handle == C_NULL && error("Failed to initialize")
    handle
end

function process(handle, input)
    result = ccall((:enaction_engine_process, libenaction_engine), Cint, (Ptr{Cvoid}, UInt32), handle, input)
    result
end

function cleanup(handle)
    ccall((:enaction_engine_free, libenaction_engine), Cvoid, (Ptr{Cvoid},), handle)
end
...
3. **Update FFI implementation** (`src/interface/ffi/src/*.zig`)
