...
= Proof Requirements — ENACTION_ENGINE
...
These RSR proof obligations come from the estate standards and are tracked here
where they apply to Enaction Engine:
...
| ABI-1 | Buffer pointer/nullability policy | Partial: product policy proved; Zig refinement open | `src/interface/Abi/Accelerator.idr` (`ABI-P7`); obsolete generic template remains quarantined
| ABI-2 | Production field alignment and bounds | Proved for all four accelerator layouts | `src/interface/Abi/Layout.idr`, `src/interface/Abi/Accelerator.idr` (`ABI-P1`–`ABI-P4`)
| ABI-3 | Platform type size proofs (per platform) | Partial: Zig enforces 64-bit pointers; target-specific Idris witnesses and compiler refinement remain open | `src/interface/ffi/src/accelerator.zig`; quarantined `verification/proofs/idris2/ABI/Platform.idr` is not product proof
| ABI-4 | FFI function return type proofs | Needed | `verification/proofs/idris2/ABI/Foreign.idr`
| ABI-5 | C ABI compliance (`CABICompliant`) | Partial: declared model proved; compiler refinement open | `src/interface/Abi/Layout.idr`, `src/interface/Abi/Accelerator.idr`
|===
...
| CAC-1 | Exact CAC/ESM contract-version compatibility predicate | FUNC |
  Creusot | Confirmed (`RUST-P2`); callers remain outside the claim |
  `crates/enaction-contracts/src/version.rs`, `verification/creusot/src/lib.rs`

| SNIF-1 | Product-specific wasm32 request/result/buffer invariants and
  generated-Zig refinement, if a real BEAM consumer is admitted | ABI/SEC |
  Idris2 plus an explicit refinement method | Deferred until admission |
  ADR-0021; no implementation or proof file exists
|===
