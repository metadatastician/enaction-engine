...
- GitHub shows only one repo licence: the root `LICENSE` is AGPL-3.0-or-later (so the
  sidebar reads AGPL-3.0-or-later); **both** canonical texts live in `LICENSES/`
  (`AGPL-3.0-or-later.txt` + `CC-BY-SA-4.0.txt`) and prose is CC-BY-SA-4.0 by its
  per-file header, not a second root licence file.
- Fallback (platform-required only): AGPL-3.0-or-later with a comment explaining why.
...
- Copyright: `Copyright (c) 2026 Jonathan D.A. Jewell (metadatastician) <jonathan.jewell@gmail.com>`
