...
#
# The RSR template ships this as a bare `find`, which contradicts the contract
# root-allow.txt states ("Anything tracked at root that is not in this list is
# drift"): a plain filesystem scan also sees build output. Any repo with a
# root-level build directory — `target/` for Cargo, `node_modules/`, `_build/`
# for Mix — therefore fails this gate the moment someone builds before running
# it, and the tempting "fix" is to allowlist an artifact directory that must
# never be committed.
#
